...
import { StyleSheet, View, Text, SafeAreaView, StatusBar, TouchableOpacity, Linking } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { floodService } from './src/services/floodService';
...
const SafeLocationCard = () => {
  const handleNavigate = async () => {
    try {
      // Coordinates for the safe location
      const safeLocation = {
        latitude: 2.341594633163643,
        longitude: 111.84368477679566
      };
      
      // Get the Google Maps URL for navigation
      const mapsUrl = floodService.getGoogleMapsUrl(safeLocation);
      ...
      if (supported) {
        ...
        console.error("Don't know how to open URI: " + mapsUrl);
      }
    } catch (error) {
    ...
    <TouchableOpacity onPress={handleNavigate}>
      <View style={styles.safeLocationCard}>
        ...
        <View style={styles.safeStatusContainer}>
          ...
          <Text style={styles.safeStatusText}>Safe Location</Text>
        </View>
        <Text style={styles.safeLocationDescription}>
          Tap to navigate to the safe location
        </Text>
      </View>
    </TouchableOpacity>
  );
